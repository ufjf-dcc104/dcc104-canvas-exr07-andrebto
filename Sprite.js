...
Sprite.prototype.mover = function (map, dt) {
  ...
  if((this.vx>0 && map.cells[this.gy][this.gx+1]==0)||(this.vx>0 && map.cells[this.gy][this.gx+1]==4)){
    this.x += Math.min((this.gx+1)*map.SIZE - (this.x+this.SIZE/2),this.vx*dt);
  } else if((this.vx <0 && map.cells[this.gy][this.gx-1]==0)||(this.vx <0 && map.cells[this.gy][this.gx-1]==4)){
      this.x += Math.max((this.gx)*map.SIZE - (this.x-this.SIZE/2),this.vx*dt);
    }
  else {
    this.x = this.x + this.vx*dt;
  }
  if((this.vy >0 && map.cells[this.gy+1][this.gx]==0)||(this.vy >0 && map.cells[this.gy+1][this.gx]==4)){
    this.y += Math.min((this.gy+1)*map.SIZE - (this.y+this.SIZE/2),this.vy*dt);
  } else if((this.vy<0 && map.cells[this.gy-1][this.gx]==0)||(this.vy<0 && map.cells[this.gy-1][this.gx]==4)){
